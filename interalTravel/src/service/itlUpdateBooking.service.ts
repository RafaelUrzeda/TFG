import { actualizarAmadeusSession, deleteItl_update_reservas_logs, establecerCorrecto, establecerIncorrecto, startRegister, updateItl_update_reservas_logs, } from '../database/infoSol.database';
...
const fullItlUpdateBookingService = async (idReserva: number, token: string, localizador: string): Promise<string> => {
	...
	const booking = await finishBooking(itlBooking, amaData.amadeussession, token);
	...
	await postFinishSessionAma(token, amaData.amadeussession);
	await deleteItl_update_reservas_logs(idReserva);
	await updateItl_update_reservas_logs(itlBooking);
...
const getPNR = async (amaData: TransactionInfo) => {
	let response;
...
export const sanitizeAmadeusErrors = async (amaData: TransactionInfo) => {
	const newItlBooking: Booking = {};
	newItlBooking.ignorePNR = true;

	const response = await getAmadeus(newItlBooking, amaData.token, amaData.amadeussession);
	amaData.amadeussession = response?.headers.amadeussession;
};

const executeCommandList = async (nuCommands: string[], token: string, amaData: TransactionInfo): Promise<any> => {
