...
import { dataBooking, dataFlight, dataPaxes, dataSsr, updateTables } from './database.service';
    ...
    if (!allOkInBookingProcess) {
        ...
        if (!finalizada) {
            ...
            if (itlBooking.localizador) {
                ...
                try {
                    for (let i = 0; i < 2; i++) {
                        const booking = await getAmadeusPNR(itlBooking.localizador || '');
                ...
                await finalizeBooking(itlBooking, idBooking, bookingData, amadeusSession || '');
            } else {
                await postFinishSessionAma( amadeusSession || '');
            }
        }
